        ...
        .then((promise)=>{
            ...
            mealListe=[];
             for(let x=0 ; x<promiseData.length; x++){
                ...
                strIngredient += `<p>${mealListe[x][`strIngredient${i}`]}:${mealListe[x][`strMeasure${i}`]}<input onclick="checkList(event)" id="checkbox${i}" type="checkbox" class="checkbox"><button id="buttonId${i}" onclick="addList(event)" class="addbutton">Add</button></p>`;
            }
                }
...
function addList(e){
    ...
    mangelElement=addBox.parentElement.firstChild;
